...
$(document).ready(function () {
    ...
    var table = $('#operacoes').DataTable({
        footerCallback: function (row, data, start, end, display) {
            ...
            // Update footer
            $(api.column(3).footer()).html('R$ ' + pageTotal.toFixed(2));
        },
        ...
        buttons: [ // Botões
            {
                extend: 'excelHtml5',
                text: '<i class="bi bi-filetype-xlsx icon"></i>',
                className: 'btn btn-success',
            },
            {
                extend: 'pdfHtml5',
                text: '<i class="bi bi-filetype-pdf icon"></i>',
                className: 'btn btn-danger',
            },],
        aaSorting: [], // Não vai organizar de forma automatica, mas sim pelo backend
        pageLength: 10, // Nº de itens por página
        columnDefs: [{
            targets: [5, 6],
            orderable: false
